...
namespace Axis.Dia.Tests.IO.Binary.Serializers
{
    ...
    public class StringPayloadSerializerTests
    {
        ...
        {
            var nullValue = StringValue.Null();
            var result = StringPayloadSerializer.Serialize(nullValue, new Dia.IO.Binary.BinarySerializerContext());
            ...
            var text = "";
            var value = StringValue.Of(text);
            result = StringPayloadSerializer.Serialize(value, new Dia.IO.Binary.BinarySerializerContext());
            Assert.IsTrue(result.IsDataResult());
            data = result.Resolve();
            Assert.AreEqual(1, data.Length);
