...
namespace Axis.Dia.Tests.IO.Binary.Serializers
{
    ...
    public class BlobPayloadSerializerTests
    {
        ...
        {
            var nullValue = BlobValue.Null();
            var result = BlobPayloadSerializer.Serialize(nullValue, new Dia.IO.Binary.BinarySerializerContext());
            ...
            var value = BlobValue.Of(bytes);
            result = BlobPayloadSerializer.Serialize(value, new Dia.IO.Binary.BinarySerializerContext());
            Assert.IsTrue(result.IsDataResult());
            data = result.Resolve();
            Assert.AreEqual(1, data.Length);
