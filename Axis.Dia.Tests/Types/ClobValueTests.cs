using Axis.Dia.Contracts;
using Axis.Dia.Types;

namespace Axis.Dia.Tests.Types
{
    [TestClass]
    public class ClobValueTests
    {
        [TestMethod]
        public void Creation_Tests()
        {
            #region null value
            var value = new ClobValue();

            Assert.AreEqual(null, value.Value);
            Assert.IsNotNull(value.Annotations);
            Assert.AreEqual(0, value.Annotations.Length);


            value = new ClobValue(Annotation.Of("first", "second"));

            Assert.AreEqual(null, value.Value);
            Assert.IsNotNull(value.Annotations);
            Assert.AreEqual(2, value.Annotations.Length);


            value = new ClobValue(null, "first", "second");

            Assert.AreEqual(null, value.Value);
            Assert.IsNotNull(value.Annotations);
            Assert.AreEqual(2, value.Annotations.Length);
            #endregion

            #region non-null value
            value = new ClobValue("something");

            Assert.AreEqual("something", value.Value);
            Assert.IsNotNull(value.Annotations);
            Assert.AreEqual(0, value.Annotations.Length);


            value = new ClobValue("other thing", Annotation.Of("first", "second"));

            Assert.AreEqual("other thing", value.Value);
            Assert.IsNotNull(value.Annotations);
            Assert.AreEqual(2, value.Annotations.Length);


            value = new ClobValue("something", "first", "second");

            Assert.AreEqual("something", value.Value);
            Assert.IsNotNull(value.Annotations);
            Assert.AreEqual(2, value.Annotations.Length);
            #endregion

            #region implicits
            value = "other thing";

            Assert.AreEqual("other thing", value.Value);
            Assert.IsNotNull(value.Annotations);
            Assert.AreEqual(0, value.Annotations.Length);


            value = "other thing".ToArray();

            Assert.AreEqual("other thing", value.Value);
            Assert.IsNotNull(value.Annotations);
            Assert.AreEqual(0, value.Annotations.Length);


            value = new Span<char>("other thing".ToArray());

            Assert.AreEqual("other thing", value.Value);
            Assert.IsNotNull(value.Annotations);
            Assert.AreEqual(0, value.Annotations.Length);
            #endregion
        }

        [TestMethod]
        public void Type_Tests()
        {
            var value = default(ClobValue);
            Assert.AreEqual(DiaType.Clob, value.Type);
        }

        [TestMethod]
        public void Value_Tests()
        {
            var value = ClobValue.Of(null);
            Assert.IsNull(value.Value);

            value = "something";
            Assert.IsNotNull(value.Value);
        }

        [TestMethod]
        public void IsNull_Tests()
        {
            var value = ClobValue.Of(null);
            Assert.IsTrue(value.IsNull);

            value = ClobValue.Of(null, Annotation.Of("bleh"));
            Assert.IsTrue(value.IsNull);

            value = "something";
            Assert.IsFalse(value.IsNull);
        }

        [TestMethod]
        public void Annotation_Tests()
        {
            var value = new ClobValue(null, Annotation.Of("bleh", "other"));
            Assert.IsNotNull(value.Annotations);
            Assert.AreEqual(2, value.Annotations.Length);

            value = default;
            Assert.IsNotNull(value.Annotations);
            Assert.AreEqual(0, value.Annotations.Length);

            Assert.ThrowsException<ArgumentNullException>(() => new ClobValue(null, null as Annotation[]));
            Assert.ThrowsException<ArgumentException>(() => new ClobValue(null, "stuff", null));
        }

        [TestMethod]
        public void DeepCopy_Tests()
        {
            ClobValue value = default;
            var copied = value.DeepCopy();

            Assert.AreEqual(value, copied);
            Assert.AreEqual(value.Annotations.Length, copied.Annotations.Length);
            Assert.AreEqual(value.Value, copied.Value);

            value = ClobValue.Of("something", Annotation.Of("bleh"));
            copied = value.DeepCopy();

            Assert.AreEqual(value, copied);
            Assert.AreEqual(value.Annotations.Length, copied.Annotations.Length);
            Assert.AreEqual(value.Value, copied.Value);
        }

        [TestMethod]
        public void Null_Tests()
        {
            var value = ClobValue.Null();
            Assert.AreEqual(default, value);

            value = ClobValue.Null(Annotation.Of("stuff"));
            Assert.IsFalse(default(ClobValue).Equals(value));
            Assert.IsTrue(default(ClobValue).ValueEquals(value));
            Assert.AreEqual(default(ClobValue).Value, value.Value);
            Assert.AreNotEqual(default(ClobValue).Annotations.Length, value.Annotations.Length);
        }

        [TestMethod]
        public void ValueEquals_Tests()
        {
            ClobValue first = "something";
            ClobValue second = "other thing";
            ClobValue third = ClobValue.Of("something", Annotation.Of("annotated"));

            Assert.IsTrue(first.ValueEquals(first));
            Assert.IsTrue(first.ValueEquals(third));
            Assert.IsFalse(first.ValueEquals(second));
        }

        [TestMethod]
        public void EqualsTest()
        {
            ClobValue first = "something";
            ClobValue second = "other thing";
            ClobValue third = ClobValue.Of("something", Annotation.Of("annotated"));

            Assert.IsTrue(first.Equals(first));
            Assert.IsFalse(first.Equals(third));
            Assert.IsFalse(first.Equals(second));
        }
    }
}
