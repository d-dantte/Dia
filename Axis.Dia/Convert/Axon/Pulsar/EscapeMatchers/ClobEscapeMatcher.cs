...
namespace Axis.Dia.Convert.Axon.Pulsar.EscapeMatchers
{
    public class ClobEscapeMatcher : IEscapeSequenceMatcher
    {
        ...
        public bool TryMatchEscapeArgument(BufferedTokenReader reader, out char[] tokens)
        {
            ...
            // new line
            if (reader.Reset(position).TryNextToken(out var token)
                && ('\n' == token || '\r' == token))
                return true;
